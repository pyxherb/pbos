...
km_result_t kf_hashmap_remove(kf_hashmap_t *dest, kf_hashmap_node_t *node) {
	...
	if (node->owner_bucket->nodes == node) {
		...
		node->owner_bucket->nodes = next ? PBOS_CONTAINER_OF(kf_hashmap_node_t, list_header, next) : NULL;
	}
	...
	for (size_t i = 0; i < dest->bucket_num; ++i) {
		...
		if (bucket->nodes) {
			kf_list_foreach(i, &bucket->nodes[0].list_header) {
				dest->node_free(PBOS_CONTAINER_OF(kf_hashmap_node_t, list_header, i));
			}
		}
	}
...
kf_hashmap_node_t *kf_hashmap_find(kf_hashmap_t *dest, const void *key) {
	...
	kf_list_foreach(i, &dest->buckets[index].nodes->list_header) {
		if (dest->node_cmp(PBOS_CONTAINER_OF(kf_hashmap_node_t, list_header, i), key))
			return PBOS_CONTAINER_OF(kf_hashmap_node_t, list_header, i);
	}
...
kf_hashmap_node_t *kf_hashmap_end(kf_hashmap_t *dest) {
	for (size_t i = dest->bucket_num; i; --i) {
		if (dest->buckets[i - 1].nodes)
			return PBOS_CONTAINER_OF(kf_hashmap_node_t, list_header, kf_list_end(&dest->buckets[i - 1].nodes->list_header));
	}
	return NULL;
}
...
kf_hashmap_node_t *kf_hashmap_next(kf_hashmap_node_t *cur) {
	...
	if (next)
		return PBOS_CONTAINER_OF(kf_hashmap_node_t, list_header, next);
	...
	if (dest->buckets) {
		...
		for (size_t i = 0; i < old_bucket_num; ++i) {
			...
			for (kf_list_node_t *i = &bucket->nodes->list_header; i;) {
				...
				result = _kf_hashmap_insert(dest, PBOS_CONTAINER_OF(kf_hashmap_node_t, list_header, i));

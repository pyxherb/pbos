...
PBOS_NO_SANITIZE PBOS_API void __asan_handle_no_return(void) {
	// This prevent the compiler to complain.
}

PBOS_NO_SANITIZE PBOS_API void __asan_alloca_poison(void *addr, size_t size) {
...
PBOS_NO_SANITIZE PBOS_API void __asan_loadN(void *addr, size_t size) {
	if (ki_kasan_is_byte_poisoned(addr))
		ki_kasan_report(addr, size, false, __builtin_return_address(0));
}
PBOS_NO_SANITIZE PBOS_API void __asan_storeN(void *addr, size_t size) {
	if (ki_kasan_is_byte_poisoned(addr))
		ki_kasan_report(addr, 1, true, __builtin_return_address(0));
}

#define KASAN_LOAD_STORE_NOABORT_FN(width)                                     \
	PBOS_NO_SANITIZE PBOS_API void __asan_load##width##_noabort(void *addr) {  \
		__asan_load##width(addr);                                              \
	}                                                                          \
	PBOS_NO_SANITIZE PBOS_API void __asan_store##width##_noabort(void *addr) { \
...
PBOS_NO_SANITIZE PBOS_API void __asan_loadN_noabort(void *addr, size_t size) {
	__asan_loadN(addr, size);
}
PBOS_NO_SANITIZE PBOS_API void __asan_storeN_noabort(void *addr, size_t size) {
	__asan_storeN(addr, size);
}

#define KASAN_REPORT_LOAD_STORE_FN(width)                                             \
	PBOS_NO_SANITIZE PBOS_API void __asan_report_load##width##_noabort(void *addr) {  \
		ki_kasan_report(addr, width, false, __builtin_return_address(0));             \
	}                                                                                 \
	PBOS_NO_SANITIZE PBOS_API void __asan_report_store##width##_noabort(void *addr) { \
		ki_kasan_report(addr, width, true, __builtin_return_address(0));              \
	}
...
PBOS_NO_SANITIZE PBOS_API void __asan_report_load_n_noabort(void *addr, size_t size) {
	ki_kasan_report(addr, size, false, __builtin_return_address(0));
}
PBOS_NO_SANITIZE PBOS_API void __asan_report_store_n_noabort(void *addr, size_t size) {

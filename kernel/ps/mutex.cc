...
PBOS_API bool ps_is_mutex_locked(ps_mutex_t *mtx) {
...
PBOS_API bool ps_is_rec_mutex_locked(ps_rec_mutex_t *mtx) {
	return hal_is_spinlock_locked(&mtx->_data.spinlock);
}

PBOS_API size_t ps_get_rec_mutex_lock_times(ps_rec_mutex_t *mtx) {
	kd_dbgcheck(mtx->_data.spinlock == HAL_SPINLOCK_LOCKED, "The mutex is not locked");

...
PBOS_NODISCARD PBOS_API km_result_t ps_create_proc(
	ps_pcb_t *pcb,
	ps_proc_id_t parent);
PBOS_API ps_thread_id_t ps_create_thread(
	ps_proc_access_t access,
	ps_pcb_t *pcb,
	size_t stack_size);

PBOS_API uint16_t ps_max_proc();
...
PBOS_API ps_tcb_t *ps_alloc_tcb(ps_pcb_t *pcb);

PBOS_API km_result_t ps_thread_alloc_stack(ps_tcb_t *tcb, size_t size);
PBOS_API km_result_t ps_thread_alloc_kernel_stack(ps_tcb_t *tcb, size_t size);
PBOS_API mm_context_t *ps_mm_context_of(ps_pcb_t *pcb);

PBOS_API void ps_user_thread_init(ps_tcb_t *tcb);
PBOS_API void ps_thread_set_entry(ps_tcb_t *tcb, void *ptr);

PBOS_API ps_cpuid_t ps_get_cur_cpuid();
PBOS_API void ki_set_cur_cpuid(ps_cpuid_t cpuid);

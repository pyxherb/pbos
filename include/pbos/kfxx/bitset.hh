#ifndef _PBOS_KFXX_BITSET_HH_
#define _PBOS_KFXX_BITSET_HH_
...
namespace kfxx {
	...
	public:
		PBOS_FORCEINLINE bitset_t() {
		}
		PBOS_FORCEINLINE ~bitset_t() = default;
		...
		PBOS_FORCEINLINE uint8_t *data() const {
			return _buffer;
		}

		PBOS_FORCEINLINE void set_bit(size_t bit_index) {
			kd_assert(bit_index < N);
			_set_bit(bit_index);
		}
		...
		PBOS_FORCEINLINE void fill_set(size_t bit_index, size_t num_bits) {
			kd_assert(bit_index < N);
			kd_assert(bit_index + num_bits <= N);

	...
	public:
		...
		[[nodiscard]] PBOS_FORCEINLINE bool _expand_to(
			...
			if constexpr (construct) {
				...
				size_t idx_last_constructed_obj;
				scope_guard scope_guard(
					[this, &idx_last_constructed_obj, new_data]() noexcept {
						for (size_t i = _length;
							i < idx_last_constructed_obj;
							++i) {
		...
		[[nodiscard]] PBOS_FORCEINLINE bool _resize(size_t length, bool force_resize_capacity) {
			...
			if (capacity_status > 0) {
				size_t new_capacity = _capacity ? (_capacity << 1) : length;
		...
		[[nodiscard]] PBOS_FORCEINLINE bool erase_range(size_t idx_start, size_t idx_end) {
			...
			const size_t postgap_length = _length - idx_end;
			const size_t new_length = _length - gap_length;

			int capacity_status = _check_capacity(new_length, _capacity);
			kd_assert(capacity_status <= 0);
			if (capacity_status < 0) {
				size_t new_capacity = _capacity >> 1,
					   new_capacity_total_size = new_capacity * sizeof(T);
				T *new_data = (T *)_allocator->alloc(new_capacity_total_size, alignof(T));

				if (!new_data)
					return false;

				if constexpr (std::is_trivially_move_assignable_v<T>) {
					memmove(new_data, _data, sizeof(T) * idx_start);
					memmove(new_data + idx_start, _data + idx_end, sizeof(T) * postgap_length);
				} else {
					scope_guard scope_guard(
						[this, new_capacity_total_size, new_data]() noexcept {
							_allocator->release(new_data, new_capacity_total_size, alignof(T));
					...
					_move_data(new_data, _data, idx_start);
					_move_data(new_data + idx_start, _data + idx_end, postgap_length);
				...
				if constexpr (std::is_trivially_move_assignable_v<T>) {
					memmove(&_data[idx_start], &_data[idx_end], postgap_length * sizeof(T));
				} else {
					for (size_t i = idx_start; i < idx_end; ++i)
						std::destroy_at<T>(&_data[i]);
					if (idx_end < _length)
						_move_data_uninitialized(&_data[idx_start], &_data[idx_end], postgap_length);
				}
			}
		...
		/// @param length _length of space to reserve.
		...
		using this_t = dynarray<T>;

	public:
		PBOS_FORCEINLINE dynarray(allocator *allocator) : _allocator(allocator), _data(nullptr) {
		}
		PBOS_FORCEINLINE dynarray(this_t &&rhs) noexcept : _allocator(std::move(rhs._allocator)), _data(std::move(rhs._data)), _length(rhs._length), _capacity(rhs._capacity) {
			rhs._data = nullptr;
			rhs._length = 0;
			rhs._capacity = 0;
		}
		...
		PBOS_FORCEINLINE allocator *get_allocator() const {

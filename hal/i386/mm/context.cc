...
km_result_t ki_mm_init_context(mm_context_t *context) {
	...
	if (KM_FAILED(result = mm_mmap(mm_get_cur_context(), pdt_vaddr, pdt_paddr, PAGESIZE, MM_PAGE_MAPPED | MM_PAGE_READ | MM_PAGE_WRITE, 0))) {
		return result;
	}
	...
	for (size_t i = 0; i < PDX_MAX + 1; ++i) {
		...
		if (pde->mask & PDE_P) {
			...
			km_unwrap_result(
				mm_mmap(context, target_ptr, UNPGADDR(pde->address), PAGESIZE, MM_PAGE_MAPPED | MM_PAGE_READ | MM_PAGE_WRITE, 0));
/*
			void *vptr = mm_kvmalloc(mm_get_cur_context(), PAGESIZE, MM_PAGE_READ | MM_PAGE_WRITE, 0);
			if (!vptr)
				return KM_RESULT_NO_MEM;
			km_unwrap_result(mm_mmap(mm_get_cur_context(), vptr, UNPGADDR(target_pde->address), PAGESIZE, MM_PAGE_MAPPED | MM_PAGE_READ | MM_PAGE_WRITE, 0));

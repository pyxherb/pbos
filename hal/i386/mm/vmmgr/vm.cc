...
static mm_pgaccess_t hn_pgmask_to_pgaccess(uint16_t mask) {
	...
	// We cannot set if a page frame is read-only on x86  :(
	if (mask & PTE_P)
		access |= MM_PAGE_MAPPED;
	if (mask & PTE_RW)
		access |= MM_PAGE_WRITE;
	if (mask & PTE_CD)
		access |= MM_PAGE_NOCACHE;
	if (mask & PTE_U)
		access |= MM_PAGE_USER;
	if (!(mask & PTE_XD))
		access |= MM_PAGE_EXEC;
	...
	for (uint16_t i = PDX(vaddr); i <= PDX(vaddr_limit); ++i) {
		...
		if (!(pde->mask & PDE_P)) {
			...
			if (!pgdir)
				...
				km_unwrap_result(hn_mm_mmap_early(context, target_ptr, pgdir, PAGESIZE, MM_PAGE_MAPPED | MM_PAGE_READ | MM_PAGE_WRITE));
			}
...
[[nodiscard]] static km_result_t hn_do_mmap(
	...
	while (i < end) {
		...
		map:
			pte = (arch_pte_t *)mm_kvmalloc(mm_get_cur_context(), PAGESIZE, MM_PAGE_READ | MM_PAGE_WRITE, 0);
			if (!pte) {
				sg.release();
				return KM_RESULT_NO_MEM;
			}
			km_unwrap_result(
				mm_mmap(mm_get_cur_context(), pte, UNPGADDR(ctxt->pdt[PDX(UNPGADDR(i))].address), PAGESIZE, MM_PAGE_MAPPED | MM_PAGE_READ | MM_PAGE_WRITE, 0));
			pte += PTX(UNPGADDR(i));
		}
...
km_result_t mm_mmap(mm_context_t *ctxt,
	...
	rescan_pgtab:
		...
		if (iteration_times) {
			...
			km_unwrap_result(hn_do_mmap(ctxt, (void *)target_ptr, UNPGADDR(ctxt->pdt[PDX(prev_pgtab_vaddr)].address), PAGESIZE, MM_PAGE_MAPPED | MM_PAGE_READ | MM_PAGE_WRITE, MMAP_NOREMAP));
		} else
			break;
	}

	km_result_t result = hn_do_mmap(ctxt, vaddr, paddr, size, access | MM_PAGE_MAPPED, flags);
...
succeeded:
	...
	if (!(flags & VMALLOC_NORESERVE)) {
		if (KM_FAILED(mm_mmap(ctxt, p_found, NULL, size, MM_PAGE_MAPPED | access, mmap_flags | MMAP_NOREMAP)))
			kd_assert(false);
	}
	return p_found;
}
	...
	switch (level) {
		case PAGING_LEVEL_PGDIR: {
			...
			if (!(pde->address = PGROUNDDOWN(
					  mm_pgalloc(MM_PHYSICAL_MEMORY_TYPE_AVAILABLE))))
				return NULL;

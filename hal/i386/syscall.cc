...
uint32_t hn_syscall_handler(uint32_t eax, uint32_t ebx, uint32_t ecx, uint32_t edx, uint32_t esi, uint32_t edi) {
	switch (eax) {
		...
		case SYSENT_PBCORE_OPEN:
			...
			return sysent_open((const char *)ebx, (size_t)ecx, edx, esi, (ps_handle_t *)edi);
		case SYSENT_PBCORE_CLOSE:
			klog_printf("Close test");
			return sysent_close((ps_handle_t)ebx, ecx);
		...
		case SYSENT_PBCORE_EXEC_CHILD:
			...
			return sysent_exec_child((ps_handle_t)ebx, (ps_handle_t)ecx, (const char*)edx, (size_t)esi, (ps_proc_id_t *)edi);
		default:

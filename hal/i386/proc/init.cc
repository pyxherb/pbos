...
void hal_prepare_ps() {
	if (!(ps_cur_proc_per_cpu = (ps_pcb_t **)mm_kmalloc(ps_cpu_num * sizeof(ps_pcb_t *), alignof(ps_pcb_t *)))) {
		km_panic("Unable to allocate current PCB pointer storage for all CPUs");
	}

	memset(ps_cur_proc_per_cpu, 0, ps_cpu_num * sizeof(ps_pcb_t *));

	if (!(ps_cur_thread_per_cpu = (ps_tcb_t **)mm_kmalloc(ps_cpu_num * sizeof(ps_tcb_t *), alignof(ps_tcb_t)))) {
		km_panic("Unable to allocate current TCB pointer storage for all CPUs");
	}

	memset(ps_cur_thread_per_cpu, 0, ps_cpu_num * sizeof(ps_tcb_t *));
}

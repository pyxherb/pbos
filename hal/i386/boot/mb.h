...
typedef struct PBOS_PACKED _mb_info_hdr_t {
	uint32_t sz_total;
	uint32_t reserved;
} mb_info_hdr_t;
...
//
typedef struct PBOS_PACKED _mbtaghdr_t {
	uint32_t type;
	uint32_t size;
} mbtaghdr_t;
...
// Boot Command Line
//
typedef struct PBOS_PACKED _mbtag_cmdline_t {
...
// Boot Loader Name
//
typedef struct PBOS_PACKED _mbtag_loader_name_t {
	// First character of the null-terminated string.
	char string;
} mbtag_loader_name_t;
...
// Modules
//
typedef struct PBOS_PACKED _mbtag_mod_t {
	uint32_t mod_start;
	uint32_t mod_end;
	// First character of the null-terminated string.
	char string;
...
//
typedef struct PBOS_PACKED _mbtag_basicmem_info_t {
	uint32_t lower;
	uint32_t upper;
...
// BIOS Boot Device
//
typedef struct PBOS_PACKED _mbtag_bootdev_t {
	uint32_t bios_dev;
	uint32_t part;
	uint32_t subpart;
} mbtag_bootdev_t;
...
//
typedef struct PBOS_PACKED _mbtag_mmap_t {
	uint32_t sz_entry;
	uint32_t ver_entry;
} mbtag_mmap_t;

// Memory Map Entry
typedef struct PBOS_PACKED _mb_mmap_entry_t {
	uint64_t base;
	uint64_t length;
	uint32_t type;
	uint32_t reserved;
...
// VBE Information
//
typedef struct PBOS_PACKED _mbtag_vbeinfo_t {
	uint16_t mode;
...
//
typedef struct PBOS_PACKED _mbtag_framebuf_info_t {
	uint64_t addr;
	uint32_t pitch;
	uint32_t width;
	uint32_t height;
...
typedef struct PBOS_PACKED _mb_framebuf_colorinfo_hdr_t {
	uint32_t color_num;
} mb_framebuf_colorinfo_hdr_t;

// Framebuffer palettes after the header.
typedef struct PBOS_PACKED _mb_framebuf_pal_hdr_t {
	uint8_t red;
	uint8_t green;
	uint8_t blue;
...
// Framebuffer Type = 1
typedef struct PBOS_PACKED _mb_framebuf_direct_rgb_colorinfo_t {
	uint8_t rfield_pos;
	uint8_t rmask_size;
	uint8_t gfield_pos;
	uint8_t gmask_size;
...
// ELF Symbols
//
typedef struct PBOS_PACKED _mbtag_elf_symbols_t {
	uint32_t size;
	uint16_t num;
	uint16_t entsize;
	uint16_t shndx;
...
// APM Table
//
typedef struct PBOS_PACKED _mbtag_apmtab_t {
	uint16_t version;
	uint16_t cseg;
	uint32_t offset;
	uint16_t cseg16;
...
//
typedef struct PBOS_PACKED _mbtag_efi_systab32_t {
	uint32_t ptr;
...
//
typedef struct PBOS_PACKED _mbtag_efi_systab64_t {
	uint64_t ptr;
...
//
typedef struct PBOS_PACKED _mbtag_smbios_tab_t {
	uint8_t major;
	uint8_t minor;
	uint8_t reserved[6];
} mbtag_smbios_tab_t;
...
// Old ACPI RSDP
//
typedef struct PBOS_PACKED _mbtag_old_rsdp_t {
	acpi_rsdp rsdp;
} mbtag_old_rsdp_t;
...
//
typedef struct PBOS_PACKED _mbtag_efi_mmap_t {
	uint32_t desc_size;
...
//
typedef struct PBOS_PACKED _mbtag_efiserv_unterm_t {
...
//
typedef struct PBOS_PACKED _mbtag_efi_himg32_t {
	uint32_t ptr;
} mbtag_efi_himg32_t;
...
//
typedef struct PBOS_PACKED _mbtag_efi_himg64_t {
	uint32_t ptr;
} mbtag_efi_himg64_t;
...
//
typedef struct PBOS_PACKED _mbtag_imgbase_t {
	uint32_t addr;
} mbtag_imgbase_t;

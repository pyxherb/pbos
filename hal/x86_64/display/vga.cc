...
void vga_clear() {
	memset((uint8_t *)((char*)mm_kernel_bottom_mapping_base_vaddr + VGA_TEXTDISP_BUFFER_OFFSET), 0, VGA_TEXTDISP_WIDTH * VGA_TEXTDISP_HEIGHT * 2);
	vga_setpos(0, 0);
}
 ...
 */
void vga_scroll() {
	uint8_t *p_topline = (uint8_t *)(((char*)mm_kernel_bottom_mapping_base_vaddr + VGA_TEXTDISP_BUFFER_OFFSET) + VGA_TEXTDISP_WIDTH * 2);
	size_t size = VGA_TEXTDISP_WIDTH * VGA_TEXTDISP_HEIGHT * 2;

	// Move all line of text forward 1 line.
	memcpy((uint8_t *)((char*)mm_kernel_bottom_mapping_base_vaddr + VGA_TEXTDISP_BUFFER_OFFSET), p_topline, size);
}
...
void vga_chwrite(unsigned char ch, uint8_t fgcolor, uint8_t bgcolor, uint8_t x, uint8_t y) {
	...
	uint8_t *dest = (uint8_t *)(((char*)mm_kernel_bottom_mapping_base_vaddr + VGA_TEXTDISP_BUFFER_OFFSET) + (y * VGA_TEXTDISP_WIDTH + x) * 2);
	dest[0] = ch;
	dest[1] = attrib;
}

...
km_result_t initcar_subnode(fs_fnode_t *parent, const char *name, size_t name_len, fs_fnode_t **file_out) {
	if (parent == initcar_dir) {
		...
		fs_fnode_t *file = PBOS_CONTAINER_OF(initcar_dir_entry_t, node_header, node)->file;
		om_incref(file);
		*file_out = file;
		return KM_RESULT_OK;
	}
...
km_result_t initcar_open(fs_fnode_t *file, fs_fcb_t **fcb_out) {
	km_result_t result;
...
km_result_t initcar_read(fs_fcb_t *fcb, char *dest, size_t size, size_t off, size_t *bytes_read_out) {
	km_result_t result;
	om::object_ptr<fs_fnode_t> file = fcb->file;
